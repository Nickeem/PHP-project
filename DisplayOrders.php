    ...
    <div style="float:right">
      ...
      <button type="submit" class="btn btn-danger btn-rounded">Log out</button>
		...
		<div class="nav-icon">
            ...
            <li><a class="active" href="DisplayOrders.php">Display Orders</a></li>
		...
		<div class="menu-top">
          ...
          <div class="linkz">
            ...
            <button type="submit" class="btn btn-block btn-lg btn-primary">View All Orders</button>
						...
						<button type="submit" class="btn btn-block btn-lg btn-primary">About</button>
						</form>
          </div>				</div>
				<div class="clearfix"> </div>
			</div>
...
<!--table -->
      ...
      $result = mysqli_query($conn, "SELECT * FROM orders WHERE Date = '$date' AND order_ready='No' ORDER BY Time DESC;");
      $count = 0;
      //Print the table contents
      if ($result->num_rows > 0)
      {
      // table based on sql code
      ...
      while($row = $result->fetch_assoc())
      {
        ...
        . $row["Time"] ."</td> <td>" . $row["order_ready"] ."</td> <td>" .
         $row["Menu_Items"] ."</td> <td>" . $row["Beverage_Items"]."</td><td>".$row['userID'] ."</td></tr>";
           ...
           echo "<tr class='danger'><th scope='row'>".$count."</th>". "<td>". $row["OrderNum"]. "</td> <td>" . $row["Date"] ."</td> <td>"
           . $row["Time"] ."</td> <td>" . $row["order_ready"] ."</td> <td>" .
            $row["Menu_Items"] ."</td> <td>" . $row["Beverage_Items"]."</td><td>".$row['userID'] ."</td></tr>";
						echo"<div class='error'> Exceeds 10 orders!";
            break;
         }
      }
